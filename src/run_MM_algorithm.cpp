...
// Compute coefficients on the quadratic terms of the surrogate function without features
// [[Rcpp::export]]
arma::mat compute_quadratic_term_signed(
    int numOfVertices,
    int numOfClasses,
    const arma::vec& alpha,
    const arma::mat& tau,
  ...
  // Get the positive adjacency matrix
  arma::sp_mat posNetwork = network;
  posNetwork.transform([=](double val) { return std::max(val, 0.0); });

  // Get the negative adjacency matrix
  arma::sp_mat negNetwork = network;
  negNetwork.transform([=](double val) { return std::min(val, 0.0); });
  negNetwork *= -1;

  // Calculate pi_pos, pi_neg, and pi_zero
  arma::mat pi_pos = (tau.t() * posNetwork * tau) / sumTaus;
  arma::mat pi_neg = (tau.t() * negNetwork * tau) / sumTaus;
  arma::mat pi_zero = 1 - pi_pos - pi_neg;

  // Remove extremely small elements in the denominators
  pi_pos.transform([=](double val) { return std::max(val, minPi); });
  pi_neg.transform([=](double val) { return std::max(val, minPi); });
  pi_zero.transform([=](double val) { return std::max(val, minPi); });

  arma::mat logPi00 = arma::log(pi_zero);

  // Calculate the quadratic coefficients
  arma::mat tauL = sumDoubleMatrixByRow(tau);

  // When D_ij = 0
  arma::mat tau_nought = -(tau.each_row() - tauL);
  arma::mat A = (logPi00 * (tau_nought.t())).t();
  LB += accu((tau.t() * tau_nought) % arma::log(pi_zero)) / 2;

  // When D_ij = 1
  arma::mat logPi11 = arma::log(pi_pos / pi_zero);
  arma::mat tauG = tau.t() * posNetwork;
  A += (logPi11 * tauG).t();
  LB += accu((tauG * tau) % logPi11) / 2;

  // When D_ij = -1
  arma::mat logPi01 = arma::log(pi_neg / pi_zero);
  arma::mat tauG_neg = tau.t() * negNetwork;
  A += (logPi01 * tauG_neg).t();
  LB += accu((tauG_neg * tau) % logPi01) / 2;
...
// A wrapper function to implement the MM algorithm without features
// [[Rcpp::export]]
Rcpp::List run_MM_without_features_signed(
    int numOfVertices,
    int numOfClasses,
    const arma::vec& alpha,
    arma::mat& tau,
  ...
  arma::mat A = compute_quadratic_term_signed(numOfVertices, numOfClasses, alpha, tau, network, LB, verbose);
